...
def make_software(seed):
    ...
    logit = 1.8*hist_fail + 0.22*fan_in + 0.15*churn - 1.8*coverage - 1.0
    logit += rng.normal(0, 0.22, n)
    p = 1/(1+np.exp(-logit))
    y = (rng.uniform(0, 1, n) < p).astype(int)
    # inject a small block of mislabeled/contradictory rows (flaky signal)
    flip = rng.choice(n, size=max(1, int(0.05*n)), replace=False)
    y[flip] = 1 - y[flip]
...
def make_robotics(seed):
    ...
    action = np.zeros((n, 2))
    for t in range(1, n):
        # control loop: action responds to previous state (feedback), state responds to previous action
        action[t] = -0.6*state[t-1, :2] + rng.normal(0, 0.3, 2)
        state[t] = 0.85*state[t-1] + 0.5*np.array([action[t-1, 0], action[t-1, 1], action[t-1].sum()])
        state[t] += rng.normal(0, 0.12, 3)
    score = 0.8*state[:, 0] - 0.6*state[:, 1] + 0.5*action[:, 0] - 0.4*action[:, 1]
    score += rng.normal(0, 0.55*np.std(score) if np.std(score) > 1e-6 else 0.3, n)
    y = (score > np.median(score)).astype(int)  # "success" = well-corrected trajectory this step
    X = np.column_stack([state, action])[1:]
    return _std(X), y[1:], "clf"
...
def op_compare(R):
    ...
    # differences AND products, so comparison can surface interaction-like structure
    return np.hstack([R, R - gm, R - R[nn], R * R[nn]])
...
def evaluate_representation(R, y, task, seed=0, want_true_fragility=False, X_for_noise=None, op_seq=None):
    # 2-fold out-of-fold evaluation: 2 model fits, but scores computed over the
    # full dataset (concatenated held-out predictions) -> much lower variance
    # than a single 70/30 split at the same compute cost.
    n = R.shape[0]
    rng = np.random.default_rng(seed)
    ...
    Rn = _std(R)
    oof_pred = np.zeros(n) if task == "reg" else np.zeros(n, dtype=int)
    ok = True
    for tr, te in [(A, B), (B, A)]:
        if task == "reg":
            model = Ridge(alpha=1.0).fit(Rn[tr], y[tr])
            oof_pred[te] = model.predict(Rn[te])
        else:
            ...
            oof_pred[te] = model.predict(Rn[te])
    if task == "reg":
        perf = max(0.0, r2_score(y, oof_pred))
    else:
        if not ok:
            perf = 0.0
        else:
...
# ---- AUC-resolution variant (classification only; regression falls back to R2) ----
from sklearn.metrics import roc_auc_score

def evaluate_representation_auc(R, y, task, seed=0):
    """Same 2-fold OOF protocol as evaluate_representation, but classification perf
    is derived from ROC-AUC on predicted probabilities instead of thresholded
    accuracy-vs-chance. AUC is continuous near 0.5 and isn't floored by a hard
    correct/incorrect count the way accuracy-vs-chance is on small samples."""
    n = R.shape[0]
    rng = np.random.default_rng(seed)
    idx = rng.permutation(n)
    half = n // 2
    ...
    Rn = _std(R)
    if task == "reg":
        return evaluate_representation(R, y, task, seed=seed)
    ...
    for tr, te in [(A, B), (B, A)]:
        if len(np.unique(y[tr])) < 2:
            ok = False; break
        model = LogisticRegression(max_iter=300).fit(Rn[tr], y[tr])
        oof_proba[te] = model.predict_proba(Rn[te])[:, 1]
    if not ok or len(np.unique(y)) < 2:
        perf = 0.0
    else:
        auc = roc_auc_score(y, oof_proba)
        perf = max(0.0, 2 * (auc - 0.5))  # rescale [0.5,1] -> [0,1], same convention as before
    return {"perf": perf, "fragility": None, "auc": None if not ok else auc}

def fitness_auc(X, y, task, op_seq, lambda_cost=1.0, seed=0, cheap_fragility=True):
    R = run_program(X, op_seq)
    ev = evaluate_representation_auc(R, y, task, seed=seed)
    cost = 0.03*len(op_seq) + 0.0008*R.shape[1]
    if cheap_fragility:
        cov = np.std(R) / (np.mean(np.abs(R)) + 1e-6)
        frag_proxy = np.clip(0.1*cov, 0, 1)
